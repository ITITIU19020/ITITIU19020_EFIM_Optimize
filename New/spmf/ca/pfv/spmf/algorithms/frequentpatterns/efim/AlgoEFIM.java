...
public class AlgoEFIM {
	...
	Map<Double, Map<Double, Double>> TwoItemSetUtilityMap = new HashMap<>();
	...
	public Itemsets runAlgorithm(int k, String inputPath,
		...
		for (Map.Entry<Double, Map<Double, Double>> outerEntry : TwoItemSetUtilityMap.entrySet()) {
			double outerKey = outerEntry.getKey();
		...
		// Map<Double, Double> combinedMap = new HashMap<>(); // Initialize with
		// combined Map

		// for (Map.Entry<Integer, Integer> entry : OneItemSetUtilityMap.entrySet()) {
		// combinedMap.put(entry.getKey().doubleValue(),
		// entry.getValue().doubleValue());
		// }

		// combinedMap.putAll(transformedDoubleMap); // Add all entries from map of
		// 2-itemsets we just converted to
		// // Map<Integer, Integer>
		// Map<Double, Double> sortedBothMap = sortByValueDescendingSingle(combinedMap);

		// for (Map.Entry<Integer, Integer> entry : OneItemSetUtilityMap.entrySet()) {
		// transformedDoubleMap.put(entry.getKey().doubleValue(),
		// entry.getValue().doubleValue());
		// }
		...
		// ----------------------------------------------------------------
		// Read each line in OG database, find unpromising items and remove if found,
		// then store all of
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(inputPath));
			...
			while ((line = reader.readLine()) != null) {
				...
				content = currenTransaction.getProjectedItems() + ":" + currenTransaction
						.getProjectedPWU() + ":" + currenTransaction.getUtility() + ":" + 0.0 + "\n";
		...
		// Merge identical transactions in the sorted modified database
		// String newFile = folderPath + File.separator + "DB_Utility_New_Merged.txt";
		// Transaction prevTrans = null;
		// int consecutiveMerge = 0;
		// try (BufferedReader reader = new BufferedReader(new FileReader(currentFile));
		// 		FileChannel channel = FileChannel.open(
		// 				Paths.get(newFile), StandardOpenOption.CREATE,
		// 				StandardOpenOption.WRITE, StandardOpenOption.APPEND)) {
		...
		// 			prevTrans = currenTransaction;

		// 		} else if (isEqualTo(currenTransaction, prevTrans)) {
		...
		// 			if (consecutiveMerge == 0) {
		// 				// copy items and their profit from the previous transaction
		// 				int itemsCount = prevTrans.items.length - prevTrans.offset;
		...
		// 				int positionProjection = currenTransaction.offset;
		// 				while (positionPrevious < itemsCount) {
		// 					utilities[positionPrevious] += currenTransaction.utilities[positionProjection];
		// 					positionPrevious++;
		// 					positionProjection++;
		// 				}

		// 				// make the sum of prefix utilities
		// 				double sumUtilities = prevTrans.prefixUtility += currenTransaction.prefixUtility;

		// 				// create the new transaction replacing the two merged transactions
		// 				prevTrans = new Transaction(items, utilities,
		// 						prevTrans.transactionUtility
		// 								+ prevTrans.transactionUtility);
		// 				prevTrans.prefixUtility = sumUtilities;
		...
		// 				// merged transaction
		// 				int positionPrevious = 0;
		// 				int positionProjected = currenTransaction.offset;
		// 				int itemsCount = prevTrans.items.length;
		// 				while (positionPrevious < itemsCount) {
		// 					prevTrans.utilities[positionPrevious] += currenTransaction.utilities[positionProjected];
		// 					positionPrevious++;
		// 					positionProjected++;
		// 				}

		// 				// make also the sum of transaction utility and prefix utility
		// 				prevTrans.transactionUtility += currenTransaction.transactionUtility;
		// 				prevTrans.prefixUtility += currenTransaction.prefixUtility;
		// 			}
		...
		// 			content = prevTrans.getProjectedItems() + ":" + prevTrans.getProjectedPWU() + ":"
		// 					+ prevTrans.getUtility() + ":"
		// 					+ prevTrans.prefixUtility + "\n";
		...
		// 	// Write the remaining transactions out to file
		// 	if (prevTrans != null) {
		// 		// transactionsPe.add(previousTransaction);

		// 		content = prevTrans.getProjectedItems() + ":" + prevTrans
		// 				.getProjectedPWU() + ":" + prevTrans.getUtility() + ":"
		// 				+ prevTrans.prefixUtility + "\n";
		...
		// 	e.printStackTrace();
		// }
		// File oldDatabase = new File(currentFile);
		// oldDatabase.delete();
	...
	private static double concatenateKeys(double a, double b) {
		...
		String concatenated = aParts[0] + bParts[0] + "." + aParts[1] + bParts[1];

		return Double.parseDouble(concatenated);
	}
		...
		for (int j = 0; j < itemsToExplore.size(); j++) {
			...
			try (BufferedReader reader = new BufferedReader(new FileReader(inputPath))) {
				...
				while ((line = reader.readLine()) != null) {
					...
					currenTransaction.prefixUtility = Double.parseDouble(line.split(":")[3]);
					// How??????
							...
							if (activateTransactionMerging
								...
								if (previousTransaction == null) {
									...
									content = previousTransaction.getProjectedItems() + ":" + previousTransaction
											...
											+ previousTransaction.prefixUtility + "\n";
	...
	public void useUtilityBinArrayToCalculateLocalUtilityFirstTime(String inputPath, int maximumTransactionCount) {
		...
		utilityBinArrayLU = new double[maxItem + 1];
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(inputPath))) {
			...
			while ((line = reader.readLine()) != null) {
				...
				for (int i = 0; i < items.length; i++) {
					...
					double itemNum = Double.parseDouble(itemsString[i]);
					double single_utility = Double.parseDouble(itemsUtilitiesString[i]);
					...
					for (int j = i + 1; j < items.length; j++) {
						double item2 = Double.parseDouble(itemsString[j]);
						double double_utility = Double.parseDouble(itemsUtilitiesString[j]);
						TwoItemSetUtilityMap.putIfAbsent(itemNum, new HashMap<Double, Double>());
						TwoItemSetUtilityMap.get(
								itemNum).put(item2,
										TwoItemSetUtilityMap.get(itemNum).getOrDefault(item2, 0.0) + single_utility
												+ double_utility);
					}
